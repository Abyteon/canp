...
impl HighPerformanceExecutor {
    ...
    fn start_io_workers(&self, mut io_task_rx: mpsc::UnboundedReceiver<(TaskMetadata, BoxedTask)>) {
        ...
        tokio::task::spawn(async move {
            ...
            while let Some((metadata, task)) = io_task_rx.recv().await {
                ...
                tokio::task::spawn(async move {
                    ...
                    tokio::time::timeout(timeout_duration, task).await
        ...
        // 使用原子操作减少锁竞争，优化性能
        {
            ...
            let new_avg_nanos = (current_avg_nanos * (total_tasks - 1) as u128 + execution_time.as_nanos() as u128) / total_tasks as u128;
    ...
    /// 启动CPU工作线程池（使用Rayon）
    fn start_cpu_workers(&self, mut cpu_task_rx: mpsc::UnboundedReceiver<(TaskMetadata, BoxedCpuTask)>) {
        let stats = Arc::clone(&self.stats);
        let semaphore = Arc::clone(&self.backpressure_semaphore);
        ...
        tokio::task::spawn(async move {
            ...
            while let Some((metadata, task)) = cpu_task_rx.recv().await {
                ...
                let batch_threshold = task_batch.last()
    ...
    async fn process_cpu_batch(
        ...
        // 基于rayon官方文档的最佳实践
        cpu_pool.install(|| {
            batch.into_par_iter().for_each_with(tx, |tx, (metadata, task)| {
                let start_time = Instant::now();
                let task_id = metadata.id;
        ...
        while let Some((_task_id, result)) = rx.recv().await {
            match result {
                Ok(execution_time) => {
                    completed += 1;
            ...
            if completed + failed >= batch_size {
                break;
            }
        }
        ...
        // 优化批量统计更新，减少锁竞争
        {
            ...
            if completed + failed > 0 {
                let total_tasks = stats.completed_tasks;
                let current_avg_nanos = stats.average_execution_time.as_nanos() as u128;
                let new_avg_nanos = (current_avg_nanos * (total_tasks - (completed + failed)) as u128 + total_time.as_nanos() as u128) / total_tasks as u128;
                stats.average_execution_time = Duration::from_nanos(new_avg_nanos as u64);
            }
        }
    }
    ...
    /// 启动高优先级任务处理器
    fn start_priority_worker(&self, mut priority_task_rx: mpsc::UnboundedReceiver<(TaskMetadata, BoxedTask)>) {
        let stats = Arc::clone(&self.stats);
        ...
        tokio::task::spawn(async move {
            ...
            while let Some((metadata, task)) = priority_task_rx.recv().await {
                ...
                tokio::time::timeout(timeout_duration, task).await
                    .map_err(|_| anyhow::anyhow!("高优先级任务超时: {}秒", timeout_duration.as_secs()))
                    .and_then(|_| Ok(()))
                    .unwrap_or_else(|e| {
                ...
                debug!("✅ 高优先级任务完成: {} - 耗时: {:?}", task_id, execution_time);
            }
        });
    }
    ...
    fn start_stats_collector(&self) {
        ...
        tokio::task::spawn(async move {
            ...
            loop {
                ...
                info!("📊 执行器状态 - 总任务: {}, 完成: {}, 失败: {}, 平均耗时: {:?}", 
                      stats.total_tasks, stats.completed_tasks, stats.failed_tasks, stats.average_execution_time);
            }
        });
    }
    ...
    /// 提交IO密集型任务
    pub async fn submit_io_task<F, T>(&self, 
        description: String, 
        priority: Priority, 
        task: F
    ) -> Result<u64>
        ...
        T: Send + 'static,
    {
        ...
        // 使用try_send避免阻塞，提高响应性
        sender.send((metadata, boxed_task))
    ...
    /// 提交CPU密集型任务
    pub async fn submit_cpu_task<F, T>(&self,
        description: String,
        priority: Priority,
        task: F
    ) -> Result<u64>
        ...
        T: Send + 'static,
    {
        ...
        self.cpu_task_tx.send((metadata, boxed_task))
    ...
    /// 批量提交IO任务
    pub async fn submit_io_batch<F, T>(&self,
    ...
    /// 批量提交CPU任务
    pub async fn submit_cpu_batch<F, T>(&self,
        tasks: Vec<(String, Priority, F)>
    ) -> Result<Vec<u64>>
    where
        F: FnOnce() -> Result<T> + Send + 'static,
    ...
    pub async fn wait_for_completion(&self) -> Result<()> {
        ...
        loop {
            ...
            if stats.total_tasks > 0 && stats.completed_tasks + stats.failed_tasks >= stats.total_tasks {
                break;
            }
...
mod tests {
    ...
    fn test_task_type() {
        ...
        assert_eq!(high_priority_task.suggested_timeout(), Duration::from_secs(60));
    ...
    async fn test_io_task_execution() {
        ...
        let task_id = executor.submit_io_task(
            "测试IO任务".to_string(),
            Priority::Normal,
            async {
                tokio::time::sleep(Duration::from_millis(100)).await;
    ...
    async fn test_cpu_task_execution() {
        ...
        let task_id = executor.submit_cpu_task(
            "测试CPU任务".to_string(),
            Priority::Normal,
            move || {
                // 模拟CPU密集型计算
    ...
    async fn test_priority_task_execution() {
        ...
        let high_priority_task_id = executor.submit_io_task(
            "高优先级任务".to_string(),
            Priority::High,
            async {
    ...
    async fn test_different_priorities() {
        ...
        // 提交不同优先级的任务
        let low_priority = executor.submit_io_task(
            "低优先级".to_string(),
            Priority::Low,
            async { Ok("低优先级完成") }
        ).await.unwrap();
        
        let normal_priority = executor.submit_io_task(
            "普通优先级".to_string(),
            Priority::Normal,
            async { Ok("普通优先级完成") }
        ).await.unwrap();
        
        let high_priority = executor.submit_io_task(
            "高优先级".to_string(),
            Priority::High,
            async { Ok("高优先级完成") }
        ).await.unwrap();
        
        let critical_priority = executor.submit_io_task(
            "关键优先级".to_string(),
            Priority::Critical,
            async { Ok("关键优先级完成") }
        ).await.unwrap();
    ...
    async fn test_error_handling() {
        ...
        // 测试IO任务错误
        let result = executor.submit_io_task(
            "错误IO任务".to_string(),
            Priority::Normal,
            async {
        ...
        // 测试CPU任务错误
        let result = executor.submit_cpu_task(
            "错误CPU任务".to_string(),
            Priority::Normal,
